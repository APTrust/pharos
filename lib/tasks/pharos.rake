...
namespace :pharos do
  ...
  task :transition_Fluctus => :environment do
    ...
    db.execute('SELECT id, name, brief_name, identifier, dpn_uuid FROM institutions') do |inst_row|
      ...
      puts "Created Institution: #{inst_row[1]}"
      ...
      db.execute('SELECT id, email, encrypted_password, reset_password_token, reset_password_sent_at, remember_created_at,
        ...
        User.create(email: u_row[1], encrypted_password: u_row[2], reset_password_token: u_row[3], reset_password_sent_at: u_row[4],
                    ...
                    phone_number: u_row[14], institution_id: inst_row[0], encrypted_api_secret_key: u_row[16])
