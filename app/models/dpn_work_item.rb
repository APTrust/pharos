class DpnWorkItem < ActiveRecord::Base
  ...
  scope :is_queued, ->(param) { where("queued_at is NOT NULL and queued_at != ''") if param == 'true' }
  scope :is_not_queued, ->(param) { where("queued_at is NULL or queued_at = ''") if param == 'true' }
