...
RSpec.describe DpnWorkItemsController, type: :controller do
  ...
  let!(:item_one) { FactoryGirl.create(:dpn_work_item, task: 'sync', node: 'aptrust', identifier: '1234') }
  let!(:item_two) { FactoryGirl.create(:dpn_work_item, task: 'ingest', node: 'chronopolis', identifier: '5678') }
  ...
  describe '#GET index' do
    describe 'for admin users' do
      ...
      it 'filters by node' do
        get :index, node: 'chronopolis', format: :json
        expect(response).to be_success
        expect(assigns(:paged_results).size).to eq 1
  ...
  describe 'POST #create' do
    ...
    it 'successfully creates the dpn item' do
      post :create, dpn_work_item: { node: 'aptrust', task: 'sync', identifier: '12345678', state: 'Active' }, format: :json
      expect(response).to be_success
      assigns(:dpn_item).node.should eq('aptrust')
    ...
    describe 'for institutional admin users' do
      ...
      it 'denies access' do
        post :create, dpn_work_item: { node: 'aptrust', task: 'sync', identifier: '12345678' }, format: :json
        expect(response.status).to eq(403)
      end
    end
  end
  ...
  describe 'GET #show' do
    describe 'for admin user' do
      ...
      it 'responds successfully with both the work item and the state item set' do
        ...
        assigns(:dpn_item).node.should eq('chronopolis')
      end
